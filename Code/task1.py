...
def pca(k, data_matrix):
    ...
    # covariance_matrix = np.cov(data_matrix)
    # values, vectors = np.linalg.eig(covariance_matrix)
    # pcaa = PCA(n_components=k, copy=False)
    # latent_semantics = pcaa.fit_transform(data_matrix)
    # S = np.diag(pcaa.explained_variance_)
    # return np.array(latent_semantics), S
...
#     km = KMeans(n_clusters=k).fit_transform(data_matrix)
#     return km
